...
class Redshift(DatabaseConnector):
    """
    A class to interact with Redshift database using PySpark
    """
    def __init__(self, spark):
        """
        Initializes the Redshift connector with a SparkSession
    ...
    def connect(self, connection_id):
        """
        Connects to Redshift database using connection details from MongoDB

        :param connection_id: Identifier of the Redshift connection in MongoDB
        :return: Dictionary containing Redshift connection details
        """
        try:
            logger.info("Connecting to redshift..")
            config = connection_id["details"]
            logger.debug(f"Connection_details: {config}")
            config.update({"driver": "com.amazon.redshift.jdbc42.Driver"})
            config.update({"url": "jdbc:redshift://"+config["host"]+":"+str(config["port"]) + "/" + config["database"]})
            conn = config.copy()
            ...
            logger.info("Connection to redshift is successfull.")
            return conn
        except Exception as e:
            logger.error(f"Failed to get Redshift connection details. Operation Completed with Exception: {str(e)}")
            raise DatabaseConnectionException("Failed to connect to redshift database.")
        
    @logger.generate
    def test_connection(self, configuration, connection):
        """
        Tests the Redshift connection by reading data from a specified table

        :param configuration: Dictionary containing configuration details
        :param connection: Dictionary containing Redshift connection details
        ...
        try:
            ...
            logger.error(f"Failed to connect to Redshift. Operation Completed with Exception: {str(e)}")
            raise DatabaseConnectionException("Failed to test redshift connection.")
        
    @logger.generate
    def fetch_data(self, configuration, connection, custom_config={}):
        """
        Fetches data from Redshift table and returns as a dictionary of DataFrames
        ...
        try:
            dfs = {}
            database, table = self.check_database(configuration["table_name"], connection)
            logger.info("Fetching the redshift database.")
            ...
            alias=configuration.get("dataframe_alias","redshift_"+database+"_"+table)
            dfs.update({configuration["df_id"]: {"df": df, "alias": alias}})
            logger.info("Fetched data successfully.")
            return dfs
        except Exception as e:
            logger.error(f"Failed to fetch data from Redshift. Operation Completed with Exception: {str(e)}")
            raise DatabaseConnectionException(f"Failed to fetch data from redshift.")
        
    @logger.generate
    def write_data(self, configuration, connection, dataframe, custom_config={}):
        """
        Writes data to Redshift database table
        ...
        try:
            ...
            logger.info(f"Write data to Redshift database table successful")
            return True
        except Exception as e:
            logger.error(f"Failed to write data to Redshift database table. Operation Completed with Exception: {str(e)}")
    ...
    def check_database(self, configuration, connection):
        ...
        try:
            ...
            logger.error(f"Failed to check redshift database: {str(e)}")
