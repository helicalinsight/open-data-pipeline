import unittest
from spark_server.database_connectors.mysql import MySQL
...
class TestMySQL(unittest.TestCase):
    # creating the instance of the class MySQL
    @classmethod
    def setUpClass(cls):
        cls.spark = TestSetup.set_up_spark()
        cls.mysql = MySQL(cls.spark)
        cls.connection_id = {
            "654879fe42a09b96f228302c": {
                "type": "database",
                "details": {
                    "type": "mysql",
                    "sourceName": "MySQL Connector",
                    "host": "152.228.161.61",
                    "port": 3306,
                    "username": "sakila",
                    "password": "p_ssW0rd",
    ...
    def test_connect(self):
        config = self.mysql.connect(self.connection_id["654879fe42a09b96f228302c"])
        self.assertIsNotNone(config["url"])

    def test_test_connection(self):
        connection = self.mysql.connect(self.connection_id["654879fe42a09b96f228302c"])
        config = {"connection_id": "654879fe42a09b96f228302c", "table_name": "actor", "df_id": "65dc3fb832b5b2a5665841fb"}
        test_result = self.mysql.test_connection(config, connection)
        self.assertEqual(test_result, True)

    def test_fetch_data(self):
        connection = self.mysql.connect(self.connection_id["654879fe42a09b96f228302c"])
        config = {"connection_id": "654879fe42a09b96f228302c", "table_name": "actor", "df_id": "65dc3fb832b5b2a5665841fb"}
        result = self.mysql.fetch_data(config, connection)["65dc3fb832b5b2a5665841fb"]
        self.assertIsNotNone(result["df"])
        self.assertGreaterEqual(result["df"].count(), 1)

    def test_fetch_data_binary_data(self):
        connection = self.mysql.connect(self.connection_id["654879fe42a09b96f228302c"])
        config = {"connection_id": "654879fe42a09b96f228302c", "table_name": "staff", "df_id": "65dc3fb832b5b2a5665841fb"}
        result = self.mysql.fetch_data(config, connection)["65dc3fb832b5b2a5665841fb"]
        self.assertIsNotNone(result["df"])
        self.assertGreaterEqual(result["df"].count(), 1)
    ...
    def test_fetch_data_with_custom_config_dbtable(self):
        connection = self.mysql.connect(self.connection_id["654879fe42a09b96f228302c"])
        config = {"connection_id": "654879fe42a09b96f228302c", "table_name": "actor_test", "df_id": "65dc3fb832b5b2a5665841fb"}
        custom_config = {"dbtable": "actor"}
        result = self.mysql.fetch_data(config, connection, custom_config)["65dc3fb832b5b2a5665841fb"]
        self.assertIsNotNone(result["df"])
        self.assertGreaterEqual(result["df"].count(), 1)
    ...
    def test_fetch_data_with_invalid_table_in_custom_config(self):
        ...
        with pytest.raises(DatabaseConnectionException) as test_function:
            self.mysql.fetch_data(config, connection, custom_config)
        self.assertEqual("Failed to fetch data.", str(test_function.value))

    def test_fetch_data_with_connection_pool(self):
        connection = self.mysql.connect(self.connection_id["654879fe42a09b96f228302d"])
        config = {"connection_id": "654879fe42a09b96f228302d", "table_name": "actor", "df_id": "65dc3fb832b5b2a5665841fb"}
        result = self.mysql.fetch_data(config, connection)["65dc3fb832b5b2a5665841fb"]
        self.assertIsNotNone(result["df"])
        self.assertGreaterEqual(result["df"].count(), 1)
    ...
    def test_write_data(self):
        ...
        result = self.mysql.write_data(config, connection, dataframe)
        self.assertTrue(result)
        config = {"connection_id": "654879fe42a09b96f228302c", "table_name": "actor1", "df_id": "65dc3fb832b5b2a5665841fc"}
        result = self.mysql.fetch_data(config, connection)["65dc3fb832b5b2a5665841fc"]
        self.assertIsNotNone(result["df"])
        self.assertGreaterEqual(result["df"].count(), 1)
    ...
    def test_write_data_with_custom_config(self):
        ...
        result = self.mysql.write_data(config, connection, dataframe, custom_config)
        self.assertTrue(result)
        config = {"connection_id": "654879fe42a09b96f228302c", "table_name": "actor1", "df_id": "65dc3fb832b5b2a5665841fc"}
        result = self.mysql.fetch_data(config, connection)["65dc3fb832b5b2a5665841fc"]
        self.assertIsNotNone(result["df"])
    ...
    def test_check_database(self):
        connection = self.mysql.connect(self.connection_id["654879fe42a09b96f228302c"])
        configuration = "actor"
        database, table = self.mysql.check_database(configuration, connection)
