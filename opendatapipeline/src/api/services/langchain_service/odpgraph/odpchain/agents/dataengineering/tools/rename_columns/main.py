...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.rename_columns.toolkit import RenameColumnsToolkit
...
class RenameColumnsAgent(BaseAgent):
    """This class represents an agent for renaming columns in data using a language model. It utilizes
    configuration from a `Config` class, integrates with an `Ollama` language model, and utilizes tools
    from a `RenameColumnsToolkit` for its operations.
    ...
    def __init__(self,user_id,chat_id, session) -> None:
        """Constructor method to initialize the RenameColumnsAgent with the given parameters and set up
        the language model and tools for column renaming.
        ...
        toolkit = RenameColumnsToolkit.from_aod_api_wrapper(aod,user_id,chat_id, session)
        toolkit.get_tools()
    ...
    def agent(self):
        """Returns the initialized agent for renaming columns.

        :return: The agent instance used for column renaming tasks.
        :rtype: Agent
        """
        return self.agent