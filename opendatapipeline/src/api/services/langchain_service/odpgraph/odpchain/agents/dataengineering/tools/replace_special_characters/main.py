...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.replace_special_characters.toolkit import ReplaceSpecialCharactersToolkit
...
class ReplaceSpecialCharactersAgent(BaseAgent):
    """This class represents an agent for replacing special characters in data using a language model. It utilizes
    configuration from a `Config` class, integrates with an `Ollama` language model, and utilizes tools
    from a `ReplaceSpecialCharactersToolkit` for its operations.
    ...
    def __init__(self,user_id,chat_id, session) -> None:
        """Constructor method to initialize the ReplaceSpecialCharactersAgent with the given parameters and set up
        the language model and tools for replacing special characters.
        ...
        toolkit = ReplaceSpecialCharactersToolkit.from_aod_api_wrapper(aod, user_id, chat_id, session)
        toolkit.get_tools()
    ...
    def agent(self):
        """Returns the initialized agent for replacing special characters.

        :return: The agent instance used for special character replacement tasks.
        :rtype: Agent
        """
        return self.agent