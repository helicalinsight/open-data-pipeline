...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.uppercase.toolkit import UpperCaseToolkit
...
class UpperCaseAgent(BaseAgent):
    """Agent for performing uppercase transformations on data.

    The `UpperCaseAgent` class utilizes the Langchain framework to manage interactions and perform
    uppercase transformations on data using a specified language model. It initializes the agent with
    the appropriate configuration, including memory management and tool integration.
    ...
    def __init__(self, user_id, chat_id, session) -> None:
        ...
        toolkit = UpperCaseToolkit.from_aod_api_wrapper(aod,user_id,chat_id, session)
        toolkit.get_tools()
    ...
    def agent(self):
        ...
        :return: The initialized agent for performing uppercase transformations.
        :rtype: object
        """
        return self.agent
