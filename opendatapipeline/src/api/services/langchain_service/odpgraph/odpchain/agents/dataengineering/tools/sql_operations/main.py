...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.sql_operations.toolkit import SQLOperationsToolkit
...
class SQLOperationsAgent(BaseAgent):
    """An agent for performing SQL operations using a language model.

    This agent is configured with a language model from the `Config` class and utilizes tools from
    the `SQLOperationsToolkit` for managing SQL operations. It integrates with `DataEngineeringWrapper` 
    for API interactions and initializes the agent with settings tailored for SQL tasks.
    ...
    def __init__(self,user_id,chat_id, session) -> None:
        """Constructor method to initialize the SQLOperationsAgent with the given parameters
        and set up the language model and tools for SQL operations.
    ...
    def agent(self):
        """Returns the initialized agent for SQL operations.

        :return: The agent instance configured for SQL tasks.
        :rtype: Agent
        """
        return self.agent
