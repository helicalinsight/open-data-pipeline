...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.sort.toolkit import SortToolkit
...
class SortAgent(BaseAgent):
    """An agent for performing sorting operations on data using a language model. This agent utilizes configuration
    from a `Config` class, integrates with an `Ollama` language model, and employs tools from a `SortToolkit` to 
    carry out sorting tasks.
    ...
    def __init__(self,user_id,chat_id, session) -> None:
        """Constructor method to initialize the SortAgent with the given parameters and set up
        the language model and tools for sorting operations.
        ...
        toolkit = SortToolkit.from_aod_api_wrapper(aod,user_id,chat_id, session)
        toolkit.get_tools()
    ...
    def agent(self):
        """Returns the initialized agent for sorting operations.

        :return: The agent instance used for sorting tasks.
        :rtype: Agent
        """
        return self.agent
