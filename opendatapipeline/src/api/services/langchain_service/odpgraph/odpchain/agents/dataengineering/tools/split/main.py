...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.split.toolkit import SplitToolkit
...
class SplitAgent(BaseAgent):
    """An agent for performing split operations on data using a language model. 

    This agent is configured with a language model from the `Config` class and utilizes tools from the 
    `SplitToolkit` for managing split operations. It integrates with `DataEngineeringWrapper` for API interactions 
    and initializes the agent with settings tailored for data splitting tasks.
    ...
    def __init__(self, user_id, chat_id, session) -> None:
        """Constructor method to initialize the SplitAgent with the given parameters and set up
        the language model and tools for split operations.
        ...
        toolkit = SplitToolkit.from_aod_api_wrapper(aod,user_id,chat_id, session)
        toolkit.get_tools()
    ...
    def agent(self):
        """Returns the initialized agent for split operations.

        :return: The agent instance configured for split tasks.
        :rtype: Agent
        """
        return self.agent