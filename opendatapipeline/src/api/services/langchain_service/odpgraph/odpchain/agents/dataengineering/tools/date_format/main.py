...
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.date_format.toolkit import DateFormatToolkit
...
class DateFormatAgent(BaseAgent):
    """
    An agent for handling date format operations.

    This agent initializes a LangChain agent configured for date format operations. It sets up the necessary components 
    such as the language model, memory, and toolkit for processing date-related tasks.
    ...
    Attributes:
        agent (LangChainAgent): The LangChain agent configured for handling date format tasks.
        
    Methods:
        agent() -> LangChainAgent:
            Returns the initialized LangChain agent instance.
    """
    def __init__(self, user_id, chat_id, session) -> None:
        ...
        toolkit = DateFormatToolkit.from_aod_api_wrapper(aod, user_id, chat_id, session)
        toolkit.get_tools()
    ...
    def agent(self):
        ...
        Returns:
            LangChainAgent: The LangChain agent configured for date format tasks.
        """
        return self.agent
