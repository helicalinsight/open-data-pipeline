...
import yaml
import re
from src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.lowercase.prompt import get_prompt

llm = get_chat_model()
chat_id = "123"
session = "123"

@patch('src.api.services.langchain_service.odpgraph.odpchain.agents.dataengineering.tools.lowercase.prompt.get_columns')
...
user_input = """
- input: Convert "address" and "fullname" columns to lowercase
  expected_output: {"columns": ["address", "fullname"], "extra_info": null}

- input: Convert "fullname", "address", "firstname" and "lastname" columns to lowercase
  expected_output: {"columns": ["fullname", "address", "firstname" , "lastname"], "extra_info": null}

- input: Convert "name" column to lower
  expected_output: {"columns": ["name"], "extra_info": null}

- input: Convert "email" column to lower case
  expected_output: {"columns": ["email"], "extra_info": null}

- input: Convert "department" column to lower
  expected_output: {"columns": ["department"], "extra_info": null}

- input: Convert 1 to lower
  expected_output: {"columns": [1], "extra_info": "Numeric data cannot be converted to lower case."}
