...
class MongoOperations:
    ...
    @log.generate
    def get_history_schedule_and_user(self, schedule_id, user_id):
        try:
            schedule_document = self.db.schedule.find_one({"user_id": user_id, "_id": ObjectId(schedule_id)})
            if schedule_document:
                pipeline = schedule_document.get("pipeline", [])
